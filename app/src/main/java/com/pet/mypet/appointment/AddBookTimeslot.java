...
import com.pet.mypet.account.Otp_Activity;
...
public class AddBookTimeslot extends AppCompatActivity implements ResponceQueues, ClickAdapter {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        imageString= AddAppointment.imageString;
...
//  if (PrefrenceUtils.readBoolean(context,PrefrenceUtils.PREF_ADDRESSESAVED,false)==false){
...
//                    else
                ...
                } else if (clinic_type.equalsIgnoreCase("0")) {
                    if ((!booked_date.equalsIgnoreCase("")) && (!booked_time.equalsIgnoreCase(""))) {
                        getDistanceamount();
...
//                                add_lat = gpsTracker.latitude + "";
//                                add_long = gpsTracker.longitude + "";
//
//
//                                Log.e("HELLOTAG_URL", "clinic"+ " "+add_lat+ " "+add_long);
//
//                            }
//                        }
//                        else{
...
//                        }
    ...
    private void saveappointmenturl(String amount) {
        JSONObject paramObject = new JSONObject();
        try {
            ...
            paramObject.put("cardtype", card_mode);
        }
        catch (Exception e){
...
for (int i=0;i<jsonArray.length();i++){
    ...
    arrayListtime.add(timeModel);
            ...
            try {
                ...
                jsonObject=jsonArray.getJSONObject(0);
                    ...
                    if (jsonObject.has("requestNo")) {
                        String code = jsonObject.getString("confirmationCode");
                        String requestNo = jsonObject.getString("requestNo");

                        startActivityForResult(new Intent(context, CheckoutUIActivity.class).putExtra("amount", booking_amount)
                                .putExtra("confirmationCode", code).putExtra("REQ_id", requestNo).putExtra("cardtype", card_mode)
                                .putExtra("url", Cons.URL_VERIFYPAYMENTANDDONEAPPOINTMENT), 10);
            ...
            try{
                ...
                if (jsonObject1.getString("message").equals("An error has occurred.")){
...
//                }
    ...
    else if (url.contains(Cons.URL_GETDISTANCEAMOUNT)){
            try {
                JSONArray jsonArray = new JSONArray(responce);
                JSONObject jsonObject=jsonArray.getJSONObject(0);
                booking_amount=jsonObject.getString("amount");
                Log.e("soundamount",booking_amount);
                showAlert(booking_amount);
//                saveappointmenturl(amount);
            }
            catch (Exception e){
...
if (resultCode==RESULT_OK) {
    ...
    temp_Address="done";
            ...
            if (resultCode==RESULT_CANCELED) {
                Toast.makeText(context, "Something went wrong, payment failed!", Toast.LENGTH_LONG).show();
                Intent intent = new Intent();
    ...
    private void requestPermission() {
        ActivityCompat.requestPermissions(AddBookTimeslot.this, new
            ...
            case RequestPermissionCode:
                if (grantResults.length> 0) {
                    ...
                    if (StoragePermission) {
                        Toast.makeText(AddBookTimeslot.this, "Permission Granted",
...
//                         requestPermission();
//                     }
                        Toast.makeText(AddBookTimeslot.this,"Permission Denied",Toast.LENGTH_LONG).show();
                        finish();
                    }
                }
                break;
    ...
    private void getDistanceamount() {
        JSONObject paramObject = new JSONObject();
        try{
        paramObject.put("latitude",add_lat);
        paramObject.put("longitude",add_long);
        ...
        makeHttpCall(Cons.URL_GETDISTANCEAMOUNT, "POST", paramObject);
    ...
    private void showAlert(final String amount) {
        final AlertDialog.Builder dialog = new AlertDialog.Builder(context);
        dialog.setTitle("Alert")
                .setMessage("Billing amount "+amount+" SAR (INCLUSIVE VAT) .Please pay to confirm your booking.")
                .setPositiveButton("Pay now", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface paramDialogInterface, int paramInt) {
                      selectcardmode(amount);
                    }
                })
                .setNegativeButton("Cancel", new DialogInterface.OnClickListener() {
                    @Override
    ...
    private void selectcardmode(final String amount) {
        final CharSequence[] options = { "Debit card", "Credit card","Cancel"};
        androidx.appcompat.app.AlertDialog.Builder builder = new androidx.appcompat.app.AlertDialog.Builder(AddBookTimeslot.this);
